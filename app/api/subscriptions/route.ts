...
interface UserWithStripeCustomerId {
...
export async function POST(req: Request) {
  try {
    ...
    let customerId = (user as { stripeCustomerId?: string | null })
      ?.stripeCustomerId;
    if (!customerId) {
...
export async function PUT(req: Request) {
  try {
    ...
    if (!subscriptionId || !priceId) {
      return new NextResponse("Subscription ID and Price ID are required", {
        status: 400,
      });
    }
