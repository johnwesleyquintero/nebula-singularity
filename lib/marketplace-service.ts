...
class MarketplaceService {
  ...
  async analyzeKeywords(keywords: string[]) {
    try {
      const cachedResults = keywords
        .map((keyword) => cacheService.getKeywordMetrics(keyword))
        .filter(Boolean);
      ...
      const uncachedKeywords = keywords.filter(
        (keyword) => !cacheService.getKeywordMetrics(keyword),
      );

      const keywordData =
        await marketplaceAPI.getKeywordMetrics(uncachedKeywords);
  ...
  async validateProductListing(listing: {
    ...
    try {
      ...
      throw new Error(
        "Failed to validate listing. Please check your input and try again.",
      );
    }
  }

  async analyzePPCCampaign(
    campaignId: string,
    ...
    try {
      ...
      throw new Error(
        "Failed to analyze PPC campaign. Please try again later.",
      );
    }
  }
}
