...
const fbaCalculatorSchema = z.object({
  ...
  shippingToAmazon: z.coerce.number().min(0, "Shipping cost cannot be negative"),
...
export function FBACalculator() {
  ...
  function calculateFBAFees(values: FBACalculatorValues) {
    ...
    const dimensionalWeight = (values.length * values.width * values.height) / 139
    ...
    if (values.length > 15 || values.width > 12 || values.height > 0.75 || billableWeight > 1) {
    ...
    if (values.length > 18 || values.width > 14 || values.height > 8 || billableWeight > 20) {
    ...
    const totalCosts = values.productCost + values.shippingToAmazon + fbaFee + referralFee
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>The selling price of your product on Amazon</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>Cost to ship your product to Amazon's fulfillment center</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
      ...
      <div>
        <Card>
          <CardHeader>
            ...
            <CardDescription>Breakdown of costs and profit for your FBA product</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {results ? (
              <>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="text-lg font-semibold">{formatCurrency(form.getValues("productPrice"))}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-sm font-medium text-muted-foreground">Product Cost</p>
                    <p className="text-lg font-semibold">{formatCurrency(form.getValues("productCost"))}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-sm font-medium text-muted-foreground">Shipping to Amazon</p>
                    <p className="text-lg font-semibold">{formatCurrency(form.getValues("shippingToAmazon"))}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-sm font-medium text-muted-foreground">FBA Fees</p>
                    <p className="text-lg font-semibold">{formatCurrency(results.fbaFees)}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-sm font-medium text-muted-foreground">Referral Fee</p>
                    <p className="text-lg font-semibold">{formatCurrency(results.referralFee)}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-sm font-medium text-muted-foreground">Total Costs</p>
                    <p className="text-lg font-semibold">{formatCurrency(results.totalCosts)}</p>
                  </div>
                </div>
                <Separator />
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className={`text-xl font-bold ${results.margin >= 0 ? "text-green-600" : "text-red-600"}`}>
              ...
              <div className="flex h-[300px] items-center justify-center">
                <p className="text-muted-foreground">Enter product details and calculate to see results</p>
              </div>
            )}
          </CardContent>
        </Card>
