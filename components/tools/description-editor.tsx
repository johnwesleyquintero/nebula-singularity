...
export function DescriptionEditor() {
    ...
    setTimeout(() => {
      ...
      keywords.forEach((keyword) => {
        ...
        if (
          ...
          !description.toLowerCase().includes(`<strong>${keyword.toLowerCase()}`)
        ) {
      ...
      if (!description.toLowerCase().includes("order now") && !description.toLowerCase().includes("buy now")) {
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Enter each feature on a new line. Include the most important benefits.
                </p>
              </div>

              <Button onClick={generateDescription} disabled={isGenerating || !productName || !productFeatures}>
                {isGenerating ? (
                  <>
              ...
              {generatedDescription ? (
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-base font-medium">Generated Description</CardTitle>
                    <Button
                      variant="ghost"
                      size="sm"
                  ...
                  <CardContent>
                    <div className="max-h-[400px] overflow-y-auto rounded border bg-muted/50 p-4">
                      <pre className="whitespace-pre-wrap text-sm">{generatedDescription}</pre>
                    </div>
                  </CardContent>
                </Card>
              ) : (
                <div className="flex h-full items-center justify-center rounded-lg border border-dashed p-8">
                  <div className="flex flex-col items-center justify-center space-y-2 text-center">
                    ...
                    <h3 className="text-lg font-medium">Generate a Description</h3>
                    <p className="text-sm text-muted-foreground">
                      Enter your product details to generate an optimized description
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-sm text-muted-foreground">
                  Enter keywords you want to include in your optimized description
                </p>
              </div>

              <Button onClick={optimizeDescription} disabled={isOptimizing || !originalDescription || !targetKeywords}>
                {isOptimizing ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    Optimizing...
            ...
            <div>
              {optimizedDescription ? (
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-base font-medium">Optimized Description</CardTitle>
                    <Button
                      variant="ghost"
                      size="sm"
                ...
                <div className="flex h-full items-center justify-center rounded-lg border border-dashed p-8">
                  <div className="flex flex-col items-center justify-center space-y-2 text-center">
                    <Wand2 className="h-8 w-8 text-muted-foreground" />
                    <h3 className="text-lg font-medium">Optimize Your Description</h3>
                    <p className="text-sm text-muted-foreground">
                      Paste your current description and target keywords to optimize
                    </p>
                  </div>
                </div>
              )}
      ...
      <Alert>
        <AlertDescription>
          HTML formatting is supported. Use tags like &lt;h2&gt;, &lt;p&gt;, &lt;ul&gt;, &lt;li&gt;, and &lt;strong&gt;
          for better formatting on Amazon.
        </AlertDescription>
      </Alert>
    </div>
  )
